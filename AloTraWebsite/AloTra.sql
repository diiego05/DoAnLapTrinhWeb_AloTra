...
USE AloTra
GO
CREATE TABLE ShipperRegistrationRequests (
    Id BIGINT IDENTITY(1,1) PRIMARY KEY,
    UserId BIGINT NOT NULL,                        -- Người gửi yêu cầu
    CarrierId BIGINT NULL,                         -- Nhà vận chuyển
    AddressId BIGINT NULL,                         -- Địa chỉ hoạt động (FK Addresses)

    VehicleType VARCHAR(20) NOT NULL,              -- BIKE, CAR,...
    VehiclePlate VARCHAR(30) NOT NULL,
    IdCardFrontUrl NVARCHAR(600) NULL,
    IdCardBackUrl NVARCHAR(600) NULL,
    DriverLicenseUrl NVARCHAR(600) NULL,
    ...
    CONSTRAINT FK_ShpReq_User FOREIGN KEY (UserId) REFERENCES Users(Id) ON DELETE CASCADE,
    CONSTRAINT FK_ShpReq_Carrier FOREIGN KEY (CarrierId) REFERENCES ShippingCarriers(Id),
    CONSTRAINT FK_ShpReq_Address FOREIGN KEY (AddressId) REFERENCES Addresses(Id)
);
GO

-- Mỗi user chỉ có 1 request shipper đang pending
CREATE UNIQUE INDEX UX_ShpReq_User_Pending
ON ShipperRegistrationRequests(UserId, Status)
...
CREATE TABLE BranchRegistrationRequests (
    Id BIGINT IDENTITY(1,1) PRIMARY KEY,
    UserId BIGINT NOT NULL,                        -- FK tới Users (người gửi yêu cầu)
    Type VARCHAR(10) NOT NULL 
        CHECK (Type IN ('CREATE','JOIN')),
    BranchId BIGINT NULL,                          -- Khi JOIN: id chi nhánh muốn quản lý

    AddressId BIGINT NULL,                         -- Khi CREATE: địa chỉ đề xuất (FK tới Addresses)
    ProposedBranchName NVARCHAR(200) NULL,         -- Tên chi nhánh đề xuất khi CREATE
    ProposedBranchPhone VARCHAR(20) NULL,         -- SĐT chi nhánh đề xuất (khác SĐT user)
    ...
    CONSTRAINT FK_BrReq_User FOREIGN KEY (UserId) REFERENCES Users(Id) ON DELETE CASCADE,
    CONSTRAINT FK_BrReq_Address FOREIGN KEY (AddressId) REFERENCES Addresses(Id),
    CONSTRAINT FK_BrReq_Branch FOREIGN KEY (BranchId) REFERENCES Branches(Id)
);
GO

-- Chặn trùng request PENDING theo loại
CREATE UNIQUE INDEX UX_BrReq_User_Type_Pending
ON BranchRegistrationRequests(UserId, Type, Status)
WHERE Status = 'PENDING';
GO
...
DROP CONSTRAINT FK__CartItems__Varia__7D0E9093;

ALTER TABLE CartItems
ADD CONSTRAINT FK__CartItems__Varia__7D0E9093
FOREIGN KEY (VariantId) REFERENCES ProductVariants(Id)
ON DELETE CASCADE;
